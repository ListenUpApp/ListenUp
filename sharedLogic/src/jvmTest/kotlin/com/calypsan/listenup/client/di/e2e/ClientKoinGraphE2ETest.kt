...
class ClientKoinGraphE2ETest :
    FunSpec({
        ...
        test("every RemoteCache RPC factory is enrolled in the invalidation set") {
            ...
            // getAll<RemoteCache>() must return exactly 31: ApiClientFactory + 30 RpcChannel<S>
            ...
            // invalidation.
            // Note: 31 is the sharedModules count (no platform-only RemoteCache impls on JVM).
            ...
            // The Story World unified event log added rpcChannel<WorldEventService>()
            // (worldEventModule) for the world_events outbox sender, resolved (not re-registered)
            // inside clientSyncModule's PendingOperationSender single, so 30 → 31.
