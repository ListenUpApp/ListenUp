 ...
 *
 * The patterns (`"MMM d, yyyy"`, `"MMMM d, yyyy"`, `"MMMM yyyy"`) are shared verbatim across the
 * platform actuals, so pinning the JVM actual's locale + timezone and asserting a known epoch →
 * known string catches any format-semantics drift introduced while modernizing the Apple actual
 * (which moved from per-call allocation to a cached `NSDateFormatter`). The semantics must not shift.
 */
class DateFormattingRoundTripTest :
    FunSpec({
        ...
        test("MMM d, yyyy renders the short form") {
            formatDateShort(epochMillis) shouldBe "Jan 15, 2026"
        }
