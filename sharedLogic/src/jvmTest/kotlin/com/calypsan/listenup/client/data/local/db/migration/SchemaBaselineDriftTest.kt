...
/**
 * Drift guard for the committed Room schema baseline (currently **v8** — the squashed v1
 * export plus nested chapters (v2), reading orders (v3), the high-water frontier (v4), the
 * `tentative_span.processId` column (v5, the trunk merge), the `books` chapter-tier-label
 * columns (v6, chapter tier vocabulary), the `entities` / `entity_bio_entries` mirrors
 * (v7, Story World Stage 2), and dual-homed `entities` with `entity_bio_entries` dropped
 * (v8, Story World Stage 2 amendment)).
 *
 * PR #1060 collapsed the client migration chain to a single committed export; the current
 * authoritative baseline is `schemas/…/ListenUpDatabase/8.json`. Nothing else asserts that
 ...
 * `8.json` — or a JSON edit that doesn't match the entities — is invisible to CI.
 *
...
class SchemaBaselineDriftTest :
    FunSpec({
        test("compiled ListenUpDatabase opens a database created from the committed 8.json baseline") {
            ...
            try {
                // Create the schema in `databasePath` FROM the committed 8.json (this also
                // writes the JSON's identity hash into room_master_table), then release it.
                helper.createDatabase(version = 8).close()
                ...
                try {
                    withClue(
                        "committed 8.json no longer matches the compiled @Entity schema — " +
                            "regenerate sharedLogic/schemas/…/ListenUpDatabase/8.json " +
