...
/**
 * Drift guard for the committed Room schema baseline (currently **v9** — the squashed v1
 ...
 * (v7, Story World Stage 2), dual-homed `entities` with `entity_bio_entries` dropped
 * (v8, Story World Stage 2 amendment), and the `world_events` / `world_event_mentions`
 * mirrors (v9, Story World unified event log)).
 *
 * PR #1060 collapsed the client migration chain to a single committed export; the current
 * authoritative baseline is `schemas/…/ListenUpDatabase/9.json`. Nothing else asserts that
                ...
                try {
                    withClue(
                        "committed 9.json no longer matches the compiled @Entity schema — " +
                            "regenerate sharedLogic/schemas/…/ListenUpDatabase/9.json " +
