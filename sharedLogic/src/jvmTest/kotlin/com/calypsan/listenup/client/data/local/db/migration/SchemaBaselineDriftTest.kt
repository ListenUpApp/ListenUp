...
/**
 * Drift guard for the committed Room schema baseline (currently **v2** — the squashed v1
 * export plus the `tentative_span.processId` column).
 *
 * PR #1060 collapsed the client migration chain to a single committed export; the current
 * authoritative baseline is `schemas/…/ListenUpDatabase/2.json`. Nothing else asserts that
