...
/**
 * Mounts the [SocialService] proxy over `/api/rpc/authed` — the bearer-gated RPC surface.
 *
...
open class KtorSocialRpcFactory(
    ...
    private var cachedService: SocialService? = null

    override suspend fun get(): SocialService =
        mutex.withLock {
    ...
    internal open suspend fun connect(): SocialService {
        val baseUrl = rpcBaseUrl()
        return rpcClient().rpc("$baseUrl/api/rpc/authed").withService<SocialService>()
    }
