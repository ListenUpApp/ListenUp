...
/**
 * Mounts the [ActivityService] proxy over `/api/rpc/authed` — the bearer-gated RPC surface.
 *
 * Mirrors [KtorShelfRpcFactory]: `rpc(url)` returns a cold
...
internal open class KtorActivityRpcFactory(
    ...
    private var cachedService: ActivityService? = null

    override suspend fun get(): ActivityService =
        mutex.withLock {
    ...
    internal open suspend fun connect(): ActivityService {
        val baseUrl = rpcBaseUrl()
        return rpcClient().rpc("$baseUrl/api/rpc/authed").withService<ActivityService>()
    }
