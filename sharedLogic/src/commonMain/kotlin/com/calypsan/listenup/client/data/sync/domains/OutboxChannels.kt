...
internal object OutboxChannels {
    ...
    // Reading-order channels predate the idempotency declaration on OutboxChannel; none declares
    // `idempotent = true` yet, so an OutcomeUnknown drop quarantines conservatively. Declaring them
    // (they are modeled on the Series/ShelfBooks templates) is a follow-up requiring a server-side
    // idempotency audit.
    val ReadingOrders =
        OutboxChannel(
            SyncDomains.READING_ORDERS.name,
            ReadingOrderUpdate.serializer(),
            setOf(OpKind.Update),
            idempotent = false,
        )
