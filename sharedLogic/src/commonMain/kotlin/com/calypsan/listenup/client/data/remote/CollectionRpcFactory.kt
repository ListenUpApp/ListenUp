...
/**
 * Mounts the [CollectionService] proxy over `/api/rpc/authed` — the bearer-gated
 * RPC surface.
 *
...
open class KtorCollectionRpcFactory(
    ...
    internal open suspend fun connect(): CollectionService {
        val baseUrl = rpcBaseUrl()
        return rpcClient().rpc("$baseUrl/api/rpc/authed").withService<CollectionService>()
    }
