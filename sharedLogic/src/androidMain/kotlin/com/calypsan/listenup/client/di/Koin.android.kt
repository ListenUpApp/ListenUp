 ...
 * `Application` (in `:sharedUI`) owns its own `startKoin { androidContext(); … }` because the
 * `koin-android` context plumbing lives there; it appends its platform modules to this list.
 */
 ...
 * Swift Export surface — exposing Koin's `Module` type there crashes the link. The Android
 * `Application` (in `:sharedUI`) appends this to its `startKoin { … }` module list.
 */
