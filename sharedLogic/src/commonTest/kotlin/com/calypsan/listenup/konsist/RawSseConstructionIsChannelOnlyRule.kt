 ...
 * extraction there were three such surfaces — the sync firehose (since retired to the RPC stream)
 * plus the two pre-auth registration streams — each with its own connect loop, its own (missing)
 * connect bound, and its own fragile parser. The one surviving SSE surface (the registration-policy
 * stream) composes over [SseConnection], which owns the one battle-tested loop: the 5s Darwin
 * connect bound, exponential reconnect, and spec-tolerant framing.
 *
