...
class ChapterGroupingTest : FunSpec({
    ...
    test("book header without part header opens a null-titled part") {
        ...
        groups.single().parts.single().title shouldBe null
        groups.single().parts.single().chapters.map { it.id } shouldBe listOf("a", "b")
    }
    ...
    test("300 chapters across 10 books group without loss") {
        val chapters = (0 until 300).map { i ->
            ch(
                id = "c$i",
                start = i * 1000L,
        ...
        groups.forEach { book ->
            ...
            book.parts.single().chapters.size shouldBe 30
        }
    }
})
