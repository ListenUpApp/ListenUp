...
func (sbq *SeriesBookQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (sbq *SeriesBookQuery) FirstIDX(ctx context.Context) int {
...
func (sbq *SeriesBookQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (sbq *SeriesBookQuery) OnlyIDX(ctx context.Context) int {
...
func (sbq *SeriesBookQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (sbq *SeriesBookQuery) IDsX(ctx context.Context) []int {
	ids, err := sbq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(seriesbook.FieldSequence).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.SeriesBook.Query().
//		Select(seriesbook.FieldSequence).
//		Scan(ctx, &v)
...
func (sbq *SeriesBookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SeriesBook, error) {
	...
	if sbq.withSeries != nil || sbq.withBook != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, seriesbook.ForeignKeys...)
	}
...
func (sbq *SeriesBookQuery) loadSeries(ctx context.Context, query *SeriesQuery, nodes []*SeriesBook, init func(*SeriesBook), assign func(*SeriesBook, *Series)) error {
	...
	nodeids := make(map[string][]*SeriesBook)
	for i := range nodes {
		if nodes[i].series_series_books == nil {
			continue
		}
		fk := *nodes[i].series_series_books
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "series_series_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sbq *SeriesBookQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*SeriesBook, init func(*SeriesBook), assign func(*SeriesBook, *Book)) error {
	...
	nodeids := make(map[string][]*SeriesBook)
	for i := range nodes {
		if nodes[i].book_series_books == nil {
			continue
		}
		fk := *nodes[i].book_series_books
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "book_series_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sbq *SeriesBookQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(seriesbook.Table, seriesbook.Columns, sqlgraph.NewFieldSpec(seriesbook.FieldID, field.TypeInt))
	_spec.From = sbq.sql
