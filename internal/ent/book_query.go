...
func (bq *BookQuery) QueryChapters() *ChapterQuery {
	query := (&ChapterClient{config: bq.config}).Query()
...
func (bq *BookQuery) QueryCover() *BookCoverQuery {
...
// QueryNarrators chains the current query on the "narrators" edge.
func (bq *BookQuery) QueryNarrators() *NarratorQuery {
	query := (&NarratorClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BookQuery) QueryLibrary() *LibraryQuery {
	query := (&LibraryClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BookQuery) QueryFolder() *FolderQuery {
	query := (&FolderClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BookQuery) WithChapters(opts ...func(*ChapterQuery)) *BookQuery {
	query := (&ChapterClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withChapters = query
	return bq
}
...
func (bq *BookQuery) WithCover(opts ...func(*BookCoverQuery)) *BookQuery {
	query := (&BookCoverClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithNarrators tells the query-builder to eager-load the nodes that are connected to
// the "narrators" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookQuery) WithNarrators(opts ...func(*NarratorQuery)) *BookQuery {
	query := (&NarratorClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withNarrators = query
	return bq
}
...
func (bq *BookQuery) WithLibrary(opts ...func(*LibraryQuery)) *BookQuery {
	query := (&LibraryClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (bq *BookQuery) WithFolder(opts ...func(*FolderQuery)) *BookQuery {
	query := (&FolderClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (bq *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	...
	if query := bq.withChapters; query != nil {
		if err := bq.loadChapters(ctx, query, nodes,
			func(n *Book) { n.Edges.Chapters = []*Chapter{} },
			func(n *Book, e *Chapter) { n.Edges.Chapters = append(n.Edges.Chapters, e) }); err != nil {
			return nil, err
		}
	}
	if query := bq.withCover; query != nil {
		if err := bq.loadCover(ctx, query, nodes, nil,
			func(n *Book, e *BookCover) { n.Edges.Cover = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := bq.withNarrators; query != nil {
		if err := bq.loadNarrators(ctx, query, nodes,
			func(n *Book) { n.Edges.Narrators = []*Narrator{} },
			func(n *Book, e *Narrator) { n.Edges.Narrators = append(n.Edges.Narrators, e) }); err != nil {
			return nil, err
		}
	}
	if query := bq.withLibrary; query != nil {
		if err := bq.loadLibrary(ctx, query, nodes, nil,
			func(n *Book, e *Library) { n.Edges.Library = e }); err != nil {
			return nil, err
		}
	}
...
func (bq *BookQuery) loadChapters(ctx context.Context, query *ChapterQuery, nodes []*Book, init func(*Book), assign func(*Book, *Chapter)) error {
	...
	query.Where(predicate.Chapter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(book.ChaptersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.book_chapters
		if fk == nil {
			return fmt.Errorf(`foreign-key "book_chapters" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "book_chapters" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (bq *BookQuery) loadCover(ctx context.Context, query *BookCoverQuery, nodes []*Book, init func(*Book), assign func(*Book, *BookCover)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Book)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.BookCover(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(book.CoverColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.book_cover
		if fk == nil {
			return fmt.Errorf(`foreign-key "book_cover" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "book_cover" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (bq *BookQuery) loadAuthors(ctx context.Context, query *AuthorQuery, nodes []*Book, init func(*Book), assign func(*Book, *Author)) error {
	...
	nids := make(map[string]map[*Book]struct{})
...
func (bq *BookQuery) loadNarrators(ctx context.Context, query *NarratorQuery, nodes []*Book, init func(*Book), assign func(*Book, *Narrator)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Book)
	nids := make(map[string]map[*Book]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(book.NarratorsTable)
		s.Join(joinT).On(s.C(narrator.FieldID), joinT.C(book.NarratorsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(book.NarratorsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(book.NarratorsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Narrator](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "narrators" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (bq *BookQuery) loadLibrary(ctx context.Context, query *LibraryQuery, nodes []*Book, init func(*Book), assign func(*Book, *Library)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Book)
	for i := range nodes {
		if nodes[i].library_library_books == nil {
			continue
		}
		fk := *nodes[i].library_library_books
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "library_library_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BookQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Book, init func(*Book), assign func(*Book, *Folder)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Book)
	for i := range nodes {
		if nodes[i].folder_books == nil {
			continue
		}
		fk := *nodes[i].folder_books
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "folder_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
