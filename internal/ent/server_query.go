...
func (sq *ServerQuery) QueryConfig() *ServerConfigQuery {
	query := (&ServerConfigClient{config: sq.config}).Query()
...
func (sq *ServerQuery) WithConfig(opts ...func(*ServerConfigQuery)) *ServerQuery {
	query := (&ServerConfigClient{config: sq.config}).Query()
...
func (sq *ServerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Server, error) {
	...
	if query := sq.withConfig; query != nil {
		if err := sq.loadConfig(ctx, query, nodes, nil,
			func(n *Server, e *ServerConfig) { n.Edges.Config = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServerQuery) loadConfig(ctx context.Context, query *ServerConfigQuery, nodes []*Server, init func(*Server), assign func(*Server, *ServerConfig)) error {
	...
	query.Where(predicate.ServerConfig(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(server.ConfigColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.server_config
		if fk == nil {
			return fmt.Errorf(`foreign-key "server_config" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "server_config" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
