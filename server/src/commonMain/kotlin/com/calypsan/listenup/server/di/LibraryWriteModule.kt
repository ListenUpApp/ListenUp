 ...
 * [LibraryWriteBroker] — the sole component permitted to write inside library folders — plus the
 * [LibraryRootProvider] that tells the broker which folders those are.
...
fun libraryWriteModule(homeDir: Path): Module =
    module {
        ...
        single<LibraryRootProvider> { SqlLibraryRootProvider(db = get()) }
        single { LibraryWriteBroker(registry = get(), journal = get(), libraryRoots = get()) }
    }
