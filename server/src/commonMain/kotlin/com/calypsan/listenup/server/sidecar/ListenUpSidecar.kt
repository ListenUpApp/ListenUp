...
data class ListenUpSidecar(
    ...
    /**
     * Per-field provenance, keyed by [com.calypsan.listenup.api.metadata.BookField] *name* — a
     * plain string on disk, never the enum ordinal, so reordering the enum can never re-point an
     * entry. Unknown names are dropped on read (forward compat).
     *
     * Only entries above the scan tier are written: scan provenance records which on-disk source
     * won *within one scan pass* and is re-derived from scratch on the next one, so persisting it
     * would restore nothing. Restoring each field at its recorded tier is what lets the one write
     * rule (`writeTier >= storedTier`) arbitrate sidecar-vs-database with no extra logic.
     */
    @SerialName("fieldProvenance") val fieldProvenance: Map<String, FieldProvenance> = emptyMap(),
 ...
 * in [ListenUpSidecar.fieldProvenance]).
 */
@Serializable
