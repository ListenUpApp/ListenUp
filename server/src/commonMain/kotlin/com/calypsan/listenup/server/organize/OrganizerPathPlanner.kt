...
object OrganizerPathPlanner {
    ...
    private fun seriesPrefixFor(
        ...
        val n = sequence?.let(::sequenceLabel) ?: return ""
        return when (style) {
    ...
    /**
     * Renders a stored series position into the folder segment a human reads: `1.0` becomes `"1"`,
     * `1.5` stays `"1.5"`. A whole number printed as a `Double` is `"1.0"`, and nobody names a
     * folder "Book 1.0 - …". Same rule as the sidecar's label, for the same reason.
     */
