...
class MetadataServiceTest :
    FunSpec({
        ...
        test("searchWithFallback falls back to US when default region returns empty") {
            withInMemoryDatabase {
                var callCount = 0
                val audible = object : FakeAudibleApi(searchResult = AppResult.Success(emptyList())) {
                    override suspend fun search(
                        ...
                        return if (region == AudibleRegion.US) AppResult.Success(listOf(searchResult("B002")))
...
private fun book(asin: String) = AudibleBook(
    asin = asin,
    title = "Test Book",
    subtitle = "",
    authors = emptyList(),
...
private fun searchResult(asin: String) = AudibleSearchResult(
    asin = asin,
    title = "Test Book",
    subtitle = "",
    authors = emptyList(),
...
private open class FakeAudibleApi(
    ...
    override suspend fun search(
        region: AudibleRegion,
        params: SearchParams,
    ): AppResult<List<AudibleSearchResult>> = run { searchCalls++; searchResult }

    override suspend fun getBook(region: AudibleRegion, asin: String): AppResult<AudibleBook?> =
        run { bookCalls++; bookResult }

    override suspend fun getChapters(
        region: AudibleRegion,
        asin: String,
    ): AppResult<List<AudibleChapter>> = run { chapterCalls++; chaptersResult }
}
...
private class FakeITunesApi(
    ...
    override suspend fun findCover(title: String, author: String): AppResult<ITunesCoverHit?> =
        run { findCoverCalls++; coverResult }
}
