 ...
 */
class Mp3ParserPropertyTest : FunSpec({
    val parser = Mp3Parser()
    val asciiText = Arb.string(minSize = 1, maxSize = 40, codepoints = Codepoint.ascii()).filter { s ->
    ...
    test("TXXX user-defined frames preserve description→value mapping (unmapped keys land in custom)") {
        runBlocking {
            val customKey = Arb.string(minSize = 4, maxSize = 12, codepoints = Codepoint.ascii())
    ...
    test("ID3v1 fallback round-trips title and artist when no ID3v2") {
        runBlocking {
            ...
            val shortAscii = Arb.string(minSize = 1, maxSize = 20, codepoints = Codepoint.ascii())
