...
internal class BookGenreWriter(
     ...
     * curator alias → built-in normalization → auto-create. See [processGenreStrings] for the
     * full contract. Suspend because the auto-create step ([GenreAutoCreator]) goes through the
     * genre substrate's `upsert`.
     */
    private suspend fun resolveGenreString(
        ...
        if (customGenreId != null) {
            db.bookGenresQueries.insertIfAbsent(bookIdStr, customGenreId)
            return
        }
        ...
        for (slug in GenreNormalizer.normalizeToSlugs(raw)) {
            val genreId = db.genresQueries.findBySlug(slug).executeAsOneOrNull() ?: continue
