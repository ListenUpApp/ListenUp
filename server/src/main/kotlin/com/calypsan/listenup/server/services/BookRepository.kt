...
class BookRepository(
    ...
    /**
     * Builds a [BookSyncPayload] from an already-fetched root [bookRow] and its
     * child lists. Shared by [readPayload] (per-id) and [readPayloads] (batched)
     * so both produce byte-identical aggregates.
     */
    private fun assembleBookPayload(
        bookRow: ResultRow,
     ...
     * groups in memory (preserving the queried `ordinal`/`path` order), and assembles
     * payloads in input-id order, skipping ids with no root row. Six grouped queries per
     * chunk instead of six per book.
     */
    override suspend fun readPayloads(idStrs: List<String>): List<BookSyncPayload> {
