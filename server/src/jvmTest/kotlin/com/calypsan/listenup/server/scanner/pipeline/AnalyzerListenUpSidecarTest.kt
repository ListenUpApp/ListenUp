...
class AnalyzerListenUpSidecarTest :
    FunSpec({
        ...
        fun curatedSidecar(
            title: String,
            fieldProvenance: Map<String, FieldProvenance> = mapOf("TITLE" to FieldProvenance(FieldSourceKind.USER)),
            ...
            withSqlDatabase {
                audioLibrary {
                    ...
                    runTest {
                        ...
                        curation.fieldProvenance shouldBe
                            mapOf(BookField.TITLE to FieldProvenance(FieldSourceKind.USER))
        ...
        test("an unknown fieldProvenance key is dropped, never fatal — a newer writer's field") {
            withSqlDatabase {
                audioLibrary {
                    book("Author/My Book") {
                        audio("01.mp3")
                    ...
                    runTest {
                        ...
                        val sidecar =
                            curatedSidecar(
                                ...
                                fieldProvenance =
                                    mapOf(
                                        "TITLE" to FieldProvenance(FieldSourceKind.USER, at = 9L),
                                        "SOMETHING_FROM_THE_FUTURE" to FieldProvenance(FieldSourceKind.USER),
                                    ),
                            )
                        ...
                        // The scan still succeeds and the known field survives with its recorded stamp.
                        book.title shouldBe "Curated Title"
                        val curation = book.sidecarCuration
                        curation.shouldNotBeNull()
                        curation.fieldProvenance shouldBe
                            mapOf(BookField.TITLE to FieldProvenance(FieldSourceKind.USER, at = 9L))
                    }
                }
            }
        }

        test("a restored field's provider and timestamp reach the analyzed book's provenance") {
            withSqlDatabase {
                audioLibrary {
                    book("Author/My Book") {
                        audio("01.mp3")
                    ...
                    runTest {
                        val bookDir = fixture.root / "Author/My Book"
                        val enriched =
                            FieldProvenance(FieldSourceKind.ENRICHMENT, provider = "audible", at = 1_700_000_000_000L)
                        val sidecar =
                            ListenUpSidecar(
                                identity = SidecarIdentity(titleAuthor = "Curated Title / Author"),
                                metadata =
                                    SidecarCuratedMetadata(
                                        title = "Curated Title",
                                        description = "An enriched blurb.",
                                    ),
                        ...
                        book.description shouldBe "An enriched blurb."
                        // Not a flattened USER stamp: provider and timestamp survive the restore.
                        book.fieldProvenance[BookField.DESCRIPTION] shouldBe enriched
                        // A field the sidecar carries with no recorded provenance reads as a user edit.
                        book.fieldProvenance[BookField.TITLE] shouldBe FieldProvenance(FieldSourceKind.USER)
                    }
                }
            }
        }
