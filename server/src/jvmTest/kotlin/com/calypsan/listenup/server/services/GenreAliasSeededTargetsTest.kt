 ...
 * [GenreTable.findBySlug] returns a row. If an alias points at a slug no seeded genre
 * owns, the link silently no-ops and the resolver auto-creates the *original* raw
 * string as a fresh flat genre — recreating the very duplicate the alias meant to
 * collapse. This test makes that failure mode impossible to ship: it seeds the real
 * default taxonomy and asserts the alias targets are a subset of the seeded slugs.
 */
