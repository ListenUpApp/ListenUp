...
/**
 * The `data.local.db` package — Room database, DAOs, and `@Entity` types — is `:app:sharedLogic`
...
class DataLocalDbIsInternalRule :
    FunSpec({
        ...
        // `DownloadEnqueuer` seam — its sole cross-module binding (`:app:sharedUI` desktop's
        // no-arg `JvmDownloadEnqueuer`) moved into `:app:sharedLogic`'s `desktopDownloadModule`.
