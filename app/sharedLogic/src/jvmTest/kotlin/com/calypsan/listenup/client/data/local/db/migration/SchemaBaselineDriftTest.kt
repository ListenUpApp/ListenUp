...
/**
 * Drift guard for the committed Room schema baseline (currently **v7** — the Room 3 baseline plus
 ...
 * `book_series.sequence`, and the v6 → v7 `notifications` inbox table).
 *
 * The current authoritative baseline is `schemas/…/ListenUpDatabase/7.json`. Nothing else asserts
                ...
                try {
                    withClue(
                        "committed 7.json no longer matches the compiled @Entity schema — " +
                            "regenerate app/sharedLogic/schemas/…/ListenUpDatabase/7.json " +
