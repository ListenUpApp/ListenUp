...
/**
 * Drift guard for the committed Room schema baseline (currently **v8** — the Room 3 baseline plus
 ...
 * `book_series.sequence`, the v6 → v7 `notifications` inbox table, and the v7 → v8 two-tier
 * chapter-grouping columns).
 *
 * The current authoritative baseline is `schemas/…/ListenUpDatabase/8.json`. Nothing else asserts
                ...
                try {
                    withClue(
                        "committed 8.json no longer matches the compiled @Entity schema — " +
                            "regenerate app/sharedLogic/schemas/…/ListenUpDatabase/8.json " +
