...
/**
 * Drift guard for the committed Room schema baseline (currently **v4** — the Room 3 baseline plus
 * the v1 → v2 volume-boost migration, the v2 → v3 `books.normalizationGainDb` migration, and the
 * v3 → v4 per-user permission flags).
 *
 * The current authoritative baseline is `schemas/…/ListenUpDatabase/4.json`. Nothing else asserts
 ...
 * `4.json` — or a JSON edit that doesn't match the entities — is invisible to CI.
 *
                ...
                try {
                    withClue(
                        "committed 4.json no longer matches the compiled @Entity schema — " +
                            "regenerate app/sharedLogic/schemas/…/ListenUpDatabase/4.json " +
