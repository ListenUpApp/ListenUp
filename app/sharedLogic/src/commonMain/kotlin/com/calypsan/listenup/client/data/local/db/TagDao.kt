...
internal interface BookTagDao {
     ...
     *
     * Junction rows carry `(bookId, tagId)`, so an ungated one tells a member the id of a book
     * they cannot see and how it is classified. The gate prunes rows the server stops sending.
     */
    @Query("SELECT syncId FROM book_tags WHERE deletedAt IS NULL")
     ...
     */
    @Query("SELECT syncId FROM book_tags WHERE bookId IN (:bookIds) AND deletedAt IS NULL")
     ...
     */
    @Query(
        "UPDATE book_tags SET deletedAt = :now " +
