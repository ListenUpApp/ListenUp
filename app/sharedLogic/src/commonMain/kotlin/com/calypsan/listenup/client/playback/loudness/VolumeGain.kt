...
object VolumeGain {
     ...
     */
    fun effectiveGainDb(measuredGainDb: Float?, bookNormalizationGainDb: Float?, userBoostDb: Float): Float =
        (measuredGainDb ?: bookNormalizationGainDb ?: 0f) + userBoostDb

    /** Scale one sample by a precomputed linear gain and hard-clamp to the format ceiling. */
    fun applySample(sample: Float, linearGain: Float): Float = (sample * linearGain).coerceIn(-1f, 1f)
}
