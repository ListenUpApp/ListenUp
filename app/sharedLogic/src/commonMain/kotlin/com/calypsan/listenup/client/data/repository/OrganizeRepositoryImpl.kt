...
/**
 * Production implementation of [OrganizeRepository].
 *
 * Every call dispatches through the [OrganizeService] [RpcChannel], which folds transport faults
 * into typed [AppResult.Failure]s, re-raises cancellation, and self-heals its own connection — so
 * there is no repository-side `try`/`catch` here.
 *
 * [observeRun] unwraps the server-pushed [Flow]<[RpcEvent]<[OrganizeRunEvent]>> into a plain
 * [Flow]<[OrganizeRunEvent]>: [RpcEvent.Data] values are emitted; [RpcEvent.Error] is logged and
 * [RpcEvent.Complete] dropped (the guard already logs errors server-side) — mirroring
