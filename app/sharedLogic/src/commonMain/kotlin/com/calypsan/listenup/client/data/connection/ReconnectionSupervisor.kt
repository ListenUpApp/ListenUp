...
/**
 * Drives automatic reconnection while the sync firehose is down.
 *
 * Reachability is derived purely from the firehose connection, and the stream client's resubscribe
 * loop only ever retries the *stored* URL — so a server that returns at a new IP/port (or after a
 * restart) can leave the client stuck offline. This supervisor closes that gap: whenever the
 * firehose is not connected, it periodically (1) re-resolves the live server URL ([reevaluate] →
 * mDNS relocation + remote fallback), (2) probes the resolved active URL with the unauthenticated
 * [InstanceRepository.verifyServer], and (3) either kicks an immediate reconnect
 * ([SyncStreamClient.reconnectNow]) when it's the same instance, or triggers a clean re-auth when
 * the server's instanceId changed. A wedged same-URL socket needs no dedicated rebuild seam here:
 * the RPC channel's generation invalidation dials a fresh connection on reconnect.
 *
 ...
 *
 * The loop is gated on a boolean "is connected" (not the raw [ConnectionState]) so the stream
 * client's Connecting↔Disconnected backoff oscillation doesn't restart it; the probe interval
 * escalates on sustained failure so a long outage doesn't sweep mDNS every few seconds.
 *
                ...
                when (val probe = instanceRepository.verifyServer(active.value)) {
                    is AppResult.Success -> {
                        ...
                        if (authSession.authState.value is AuthState.SessionLapsed) {
                            // A lapsed session can't ride reconnectNow() to recovery — the resubscribe
                            // would only surface the same auth failure again (this call was the spam
                            // amplifier, resetting the stream backoff on every successful
                            // unauthenticated probe). Keep probing slowly; the engine's auth gate
                            // resumes the firehose on re-auth.
                            interval = MAX_PROBE_INTERVAL_MS
                        } else {
                            // Same instance (or no stored id to compare): server is live — kick the
                            // resubscribe loop now. On success the connection flips to Connected and
                            // collectLatest cancels us.
                            syncStreamClient.reconnectNow()
