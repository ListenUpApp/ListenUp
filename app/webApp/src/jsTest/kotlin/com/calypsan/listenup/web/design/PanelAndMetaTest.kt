...
class PanelAndMetaTest :
    FunSpec({
        ...
        test("an untitled panel has no header at all") {
            ...
            val host = mounts.mount { Panel { Text("body") } }
        ...
        test("a flush panel drops its body padding for tables that draw their own edges") {
            val flush = mounts.mount { Panel(title = "Chapters", flush = true) { Text("t") } }
            val padded = mounts.mount { Panel(title = "Chapters") { Text("t") } }
        ...
        test("machine-produced values take the mono face, human ones do not") {
            ...
            val host = mounts.mount { MetaList(META) }
        ...
        test("the current breadcrumb entry is not a link") {
            val host = mounts.mount { Breadcrumb(listOf("Library", "Horror", "The Institute")) }
