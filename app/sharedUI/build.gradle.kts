...
// app/sharedUI/src/desktopTest (DesktopPlaybackControllerTest) and
// app/sharedUI/src/androidHostTest (BrowseTreeProviderTest).
//
...
compose.resources {
    ...
    // directory name (the renamed :app:sharedUI module would otherwise shift it
...
// explicitly invoked: ./gradlew :app:sharedUI:exportLibraryDefinitions
        ...
        if (committed.get().asFile.readText() != regenerated.readText()) {
            throw GradleException(
                "OSS license manifest is out of sync with the dependency catalog:\n" +
                    " - app/sharedUI/src/commonMain/composeResources/files/aboutlibraries.json\n" +
