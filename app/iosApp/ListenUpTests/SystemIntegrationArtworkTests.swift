...
struct SystemIntegrationArtworkTests {
    @Test func dictionaryOmitsArtworkWhenNoPath() {
        let info = NowPlayingInfo(title: "T", artist: "A", windowDurationMs: 1000, windowElapsedMs: 0, rate: 1, artworkPath: nil,
    ...
    @Test func dictionaryIncludesArtworkWhenPathExists() {
        ...
        let info = NowPlayingInfo(
            title: "T", artist: "A", windowDurationMs: 1000, windowElapsedMs: 0, rate: 1, artworkPath: url.path,
            chapterNumber: nil, chapterCount: nil)
        let dict = SystemIntegration.dictionary(from: info)
